...
del data
##  2.1为分层抽样做准备：对Y值进行分类（按电磁波谱10-3800A为U波段，3800-7800A为V波段，7800-100000A为I波段）
total_data.loc[(total_data['wavelength(A)']>=0.1)&(total_data['wavelength(A)']<100),'class'] = 'X'
total_data.loc[(total_data['wavelength(A)']>=100)&(total_data['wavelength(A)']<3800),'class'] = 'U'
total_data.loc[(total_data['wavelength(A)']>=3800)&(total_data['wavelength(A)']<7800),'class'] = 'V'
total_data.loc[(total_data['wavelength(A)']>=7800)&(total_data['wavelength(A)']<25000),'class'] = 'I'
##  2.2将未参与分类的少量样本删除
...
def RF_model(X_train, y_train, X_test, y_test, i):
    ...
    model_file_folder = '../data/output/wl/models'
    ...
    new_folder_path = '../data/output/wl/prediction_data'
...
folder_save_path = '../data/output/wl/prediction_data/'  
output_file = '../data/output/wl/total_predictions.csv'  
...
### 5.4.1计算评估指标
r2_before, rmse_before, mae_before, mape_before, smape_before, count_ae_before, count_re_before = evaluation_metrics_1(y_true_before, y_pred_before, 10, 0.1)
### 5.4.2保存评估指标
data_parameters_before_1 = pd.DataFrame({'R2': r2_before, 'RMSE': rmse_before, 'MAE': mae_before, 'MAPE': mape_before, 'SMAPE': smape_before,
                                        'Number of data within 10nm': count_ae_before,
...
print(  
        ...
        f'MAPE: {mape:.4f}%\n'
        f'SMAPE: {smape:.4f}%\n'
        f'Number of data within 10nm: {count_ae_after:.4f}%\n'
...
data_parameters_after_2 = pd.DataFrame({'R2': [r2_light_after, r2_medium_after, r2_heavy_after, ],
                                  ...
                                  'DQR':[par_light_after, par_medium_after, par_heavy_after]},
